...
internal static class MediaFileConfig
{
    public static void ConfigureMediaFile<T>(this EntityTypeBuilder<T> builder)
        where T : MediaFile
    {
        builder.Property(mediaFile => mediaFile.Path).HasColumnName("MediaFile_Path").HasConversion<SysPathToStringValueConverter>().HasMaxLength(255).IsRequired();
        builder.Property(mediaFile => mediaFile.OriginalFile).HasColumnName("MediaFile_OriginalFile").IsRequired();
