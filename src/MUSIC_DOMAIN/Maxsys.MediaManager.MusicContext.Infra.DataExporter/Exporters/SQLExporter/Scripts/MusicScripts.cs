...
namespace Maxsys.MediaManager.MusicContext.Infra.DataExporter.SQLValuesScript.Scripts
{
    public class MusicScripts : InsertScriptBase<Song>
    {
        ...
        protected override string InsertValuesScript(Song obj)
            => $"({Value(obj.Id)}, {Value(obj.FullPath.AbsolutePath)}, {Value(obj.OriginalFileName)}, {Value(obj.FileSize)}, {Value(obj.CreatedAt)}, {Value(obj.LastUpdateAt)}, {Value(obj.Title)}, {Value(obj.TrackNumber)}, {Value(obj.Lyrics)}, {Value(obj.Comments)}, {Value(obj.Classification.Rating)}, {Value(obj.SongDetails.IsBonusTrack)}, {Value((byte)obj.SongDetails.VocalGender)}, {Value(obj.SongDetails.CoveredArtist)}, {Value(obj.SongDetails.FeaturedArtist)}, {Value(obj.SongProperties.Duration)}, {Value(obj.SongProperties.BitRate)}, {Value(obj.AlbumId)})";
    }
}